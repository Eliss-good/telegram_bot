...
#                 group = " ".join(select_group.find(text=True, recursive=False).split())
...
#                 lesson_part_one = " ".join(select_lesson_area_first_part.find(text=True, recursive=False).split())

#                 for select_lesson_area_second_part in select_lesson_area_first_part.find_all("span", {"class": "text-nowrap"}):

#                     lesson_part_two = " ".join(select_lesson_area_second_part.find(text=True, recursive=False).split())
...
#                         lesson_type = " ".join(select_lesson_type.find(text=True, recursive=False).split())

#                         print('lesson: ', lesson_part_one + lesson_part_two, lesson_type, group)

# get_prepod_page('https://mai.ru/education/studies/schedule/ppc.php?guid=d72d63e7-1d99-11e0-9baf-1c6f65450efa&week=1#')
