...
from bot_elements.setter.all_setters import mem_for_created_forms_set_new_question_name, \
    temp_mem_for_form_creator_add_element, mem_for_created_forms_insert_question, \
...
async def rename_question_end(message: types.Message,
                              state: FSMContext):  # newQuestionName.waiting_for_new_question_name
...
async def get_question(message: types.Message, state: FSMContext):  # form.waiting_for_question
    ...
    if data['type'] == 'msg':
        ...
        temp_mem_for_form_creator_add_element(user_id=message.chat.id,
                                              data={'question': data['question'], 'message_id': 0, 'type': 'msg'})
