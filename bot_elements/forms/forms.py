...
from bot_elements.remover.all_removers import temp_form_recipient_data_remove_element, \
    temp_mem_for_form_creator_remove_form, temp_mem_for_form_creator_remove_form_element

from bot_elements.getter.all_getters import mem_for_created_forms_get_data, temp_mem_for_form_creator_get_data, temp_form_recipient_data_get_form_id, \
    temp_form_recipient_data_get_recip_data, temp_form_recipient_data_get, temp_mem_for_form_creator_get, unique_form_id_get, \
...
async def choose_type(message: types.Message, state: FSMContext):  # name.waiting_for_name
    ...
    all_setters.temp_form_recipient_data_add_user_data(chat_id=message.chat.id,
                                                       form_name=str(message.text), type="info",
                                                       form_id=unique_form_id_get(), creator_id=message.chat.id)
...
async def get_question(message: types.Message, state: FSMContext):  # form.waiting_for_question
    ...
    if data['type'] == 'msg':
        ...
        all_setters.temp_mem_for_form_creator_add_element(user_id=message.chat.id,
                                                          data={'question': data['question'], 'message_id': 0,
                                                                'type': 'msg'})
...
async def get_options(message: types.Message, state: FSMContext):  # form.waiting_for_options
    ...
    all_setters.temp_mem_for_form_creator_add_element(user_id=message.chat.id, data={'question': user_data['question'],
...
async def add_quest_false(call: types.CallbackQuery):
    ...
    all_setters.temp_mem_for_form_creator_add_element(user_id=call.message.chat.id,
                                                      data=temp_form_recipient_data_get_recip_data(
                                                          user_id=call.message.chat.id).copy())

    all_setters.mem_for_created_forms_add_element(
        form_id=temp_form_recipient_data_get_form_id(user_id=call.message.chat.id),
        data=temp_mem_for_form_creator_get_data(call.message.chat.id).copy())

    print('mem_for_created_forms ', mem_for_created_forms_get(call.message.chat.id))
