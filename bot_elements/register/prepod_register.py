...
async def already_registered(message: types.Message, state: FSMContext):
    ...
    await message.answer('Вы уже зарегистрированы' + '\nВы: ' + str(
        registerData_get_fio(user_id=message.chat.id)) + '; ' + 'Ваша роль: ' + str(
        registerData_get_role(message.chat.id)), reply_markup=types.ReplyKeyboardRemove())
...
async def choose_fio(message: types.Message, state: FSMContext):
    ...
    registerData_add_user(user_id=message.chat.id, chosen_fio=user_data['chosen_fio'], chosen_group='prepod',
                          chosen_role=user_data['chosen_role'])
