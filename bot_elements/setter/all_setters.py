...
def temp_form_recipient_data_add_user_data(chat_id: int, form_name: str, type: str, form_id: int, creator_id: int):
    ...
    print(mem_for_created_forms, temp_form_recipient_data)
...
def registerData_add_user(user_id: int, chosen_fio: str, chosen_group: str, chosen_role: str):
    ...
    """
    if chosen_group != 'prepod' and chosen_group != 'admin' :
        con_db.add_st(chosen_fio,user_id,chosen_role,chosen_group)
    elif chosen_group == 'prepod':
        con_db.add_prepod(chosen_fio,user_id,chosen_role) """
...
def registerData_change_group_data(user_id: int, new_group: str):
    ...
    #con_db.update_data_user(con_db.find_role_us(user_id), 'group',  new_group, user_id)


def registerData_change_fio_data(user_id: int, new_fio: str):
    """Изменяет ФИО пользователя"""
    registerData[user_id]['chosen_fio'] = new_fio
    #con_db.update_data_user(con_db.find_role_us(user_id), 'name',  new_fio, user_id)