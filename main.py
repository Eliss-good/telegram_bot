...
async def make_poll(chat_id, options=['MaXImus', 'DmitRUS', 'FedoSUS', 'Ilyxus', 'ArtemOS'], is_anonymous=True, question='you are'):
...
class createPoll(StatesGroup):
    ...
    waiting_for_anwsers_json = State()
...
@dp.message_handler(state=createPoll.waiting_for_anwsers_json)
async def fio_choosen(message: types.Message, state: FSMContext):
    
    options = message.text.split(',')
    await state.update_data(options=options)
    await message.reply('Пришлите получателя')
    await createPoll.next()


@dp.message_handler(state=createPoll.waiting_for_recipient)
async def fio_choosen(message: types.Message, state: FSMContext):
    recipient = message.text.lower()
    await state.update_data(recipient=recipient)
    user_data = await state.get_data()
    # await message.reply(user_data['question'] + user_data['options'] + user_data['recipient'])
    me = 506629389
