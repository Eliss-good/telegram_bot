...
async def choose_group(message: types.Message, state: FSMContext):
    ...
    if user_data['chosen_role'] != "prepod":
        ...
        user_data = await state.get_data()
        print(user_data['chosen_role'])
        await message.answer(f"{user_data['chosen_role']} {user_data['chosen_group']}.\n")
    else:
        await message.answer(f"{user_data['chosen_role']} .\n")
    

    tg_db.reg_us(user_data['chosen_fio'], message.from_user.id, user_data['chosen_role'], name_group = user_data['chosen_group'])
    await state.finish()
...
async def send_random_value(call: types.CallbackQuery, state: FSMContext):
    ...
    tg_db.reg_us(user_data['chosen_fio'], call.message.from_user.id, user_data['chosen_role'])
    await state.finish()
...
async def cmd_poll(message: types.message):
    await message.answer('Высылаю опрос')
    options = ['MaXImus', 'DmitRUS', 'lox', 'Ilyxus', 'ArtemOS']
    chat_id = message.chat.id
    is_anonymous = True
    # open_period = 10
    question = 'you are'
